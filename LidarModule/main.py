        ...
        while not self.stop:
            ...
            if i % 40 == 39:
                ...
                if self.timer_flag:
                    ...
                    self.sound_thread.start_thread(
                        int(min_distance * 2),
                        int(min_distance_left * 2),
                        int(min_distance_right * 2),
                    )
                    sound_check = True
                    self.timer_flag = False
            ...
            while loopFlag:
                ...
                # 0x2c: fixed value of VerLen (LD19 document)
                    ...
                    # if (self.timer_flag):
                    if (
                        320 <= lidarData.Degree_angle[0] <= 360
                        or 0 <= lidarData.Degree_angle[0] <= 30
                    ):
                        check = False
                        for index in range(len(lidarData.Degree_angle)):
                            # print("check")
                            if (
                                330 <= lidarData.Degree_angle[index] <= 360
                                or 0 <= lidarData.Degree_angle[index] <= 30
                            ):
                                check = True
                                ...
                                if lidarData.Distance_i[index] < 5:
                                    ...
                                    round_number = (
                                        round(lidarData.Distance_i[index] * 2) / 2
                                    )
