...
class Transbot(object):
    ...
    def __init__(self, com='/dev/ttyUSB0', delay=.002, arm_offset=(0, 0, 0), debug=False):
        ...
        if self.ser.isOpen():
            print("Serial Opened! Baudrate=9600")
        else:
    ...
    def set_car_motion(self, velocity, angular):
        try:
            ...
            #print("motion ok!!!!!!!!!!")
            simple_cmd = [ velocity, angular]
            values_str = ','.join(map(str, simple_cmd))
            #self.ser.write(cmd)
            if(velocity != 0 or angular != 0):
            	self.ser.write(values_str.encode())
            if self.__debug:
                print("motion:", simple_cmd)
                print("motion send to arduino:", values_str)
            time.sleep(self.__delay_time)
        except:
